...
namespace VCX::Labs::Common {
    ...
    void OrbitCameraManager::ProcessInput(Engine::Camera & camera, ImVec2 const & mousePos) {
        ...
        // ✅ 关键改动：键盘平移相机需要按住 Alt（否则 WASD 全部让给角色）
        bool kbEnable = ImGui::IsItemFocused() && io.KeyAlt;

        bool panningByKeyBoard[6] = {
            kbEnable && (ImGui::IsKeyDown(ImGuiKey_W) || ImGui::IsKeyDown(ImGuiKey_UpArrow)),
            kbEnable && (ImGui::IsKeyDown(ImGuiKey_S) || ImGui::IsKeyDown(ImGuiKey_DownArrow)),
            kbEnable && (ImGui::IsKeyDown(ImGuiKey_A) || ImGui::IsKeyDown(ImGuiKey_LeftArrow)),
            kbEnable && (ImGui::IsKeyDown(ImGuiKey_D) || ImGui::IsKeyDown(ImGuiKey_RightArrow)),
            kbEnable && ImGui::IsKeyDown(ImGuiKey_Q),
            kbEnable && ImGui::IsKeyDown(ImGuiKey_E)
        };
