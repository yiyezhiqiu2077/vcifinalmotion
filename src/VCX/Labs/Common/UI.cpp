...
namespace VCX::Labs::Common {
    ...
    std::size_t UI::setupSideWindow(std::span<std::reference_wrapper<Common::ICase>> cases, std::size_t const caseId) {
        ...
        bool const singleCase  = (cases.size() <= 1);
        ...
        ImVec2 userChildSize = singleCase ? ImVec2(_layout.SideWindowSize.x, _layout.SideWindowSize.y - _layout.CaseChildPosition.y) : _layout.UserChildSize;
        ImGui::BeginChild("User Child", userChildSize, false, ImGuiWindowFlags_AlwaysUseWindowPadding);
    ...
    void UI::setupMainWindow(Common::ICase & casei) {
        ...
        ImGuiHelper::TextCentered((fmt::format("{} VIEWPORT", casei.GetName()) + '\0').c_str());
        ImGui::SameLine();
    ...
    void UI::updateFonts() {
        ...
        for (auto const & fontPath : ImGuiGetDefaultFontFileNames()) {
            assert(fontPath.data()[fontPath.size()] == '\0');
